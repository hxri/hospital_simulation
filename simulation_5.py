...
def doctor_decision(env):
    if(env.now < (datetime.datetime.combine(datetime.date.today(), datetime.time(10, 0)).timestamp())):
        return 0
    elif(env.now > (datetime.datetime.combine(datetime.date.today(), datetime.time(10, 0)).timestamp()) and env.now < (datetime.datetime.combine(datetime.date.today(), datetime.time(13, 0)).timestamp())):
...
def patient_arrival(env, doctors_1, doctors_2, registration, xray, scan, dressing, injection, pharmacy, billing):
    ...
    for i in range(NUM_PATIENTS):
        ...
        #     break
        env.process(patient_flow(env, doctors_1, doctors_2, registration, xray, scan, dressing, injection, pharmacy, billing, "Patient %d" % i))
...
def consultation_1(env, doctors_1, patient):
    global end, exit_time, num_consultation_1, cons_1_wait_times, cons_1_service_times
    ...
    cons_1_wait_times.append(total_wait_time)
    cons_1_service_times.append(processing_time)
...
def consultation_2(env, doctors_2, patient):
    global end, exit_time, num_consultation_2, cons_2_wait_times, cons_2_service_times
    ...
    cons_2_wait_times.append(total_wait_time)
    cons_2_service_times.append(processing_time)
...
# Define patient flow
def patient_flow(env, doctors_1, doctors_2, registration, xray, scan, dressing, injection, pharmacy, billing, patient):
    ...
    d3 = doctor_decision(env)
    if patient_type == 'NewOP':
        if(d0 == 0):
            total_wait_time += yield env.process(registration_process(env, registration, patient))
            if(end == 0):
                if(d3 == 0):
                    total_wait_time += yield env.process(consultation_1(env, doctors_1, patient))
                else:
                    total_wait_time += yield env.process(consultation_2(env, doctors_2, patient))
            if(end == 0):
...
for i in range(len(init.index)):
    ...
    NUM_DOCTORS_1 = init['NUM_DOCTORS'][i]
    NUM_DOCTORS_2 = init['NUM_DOCTORS'][i]
